...
class _RegisterPageState extends State<RegisterPage> {
  ...
  String? _cycleError;
  String? _departementError;
  String? _specialiteError;
  ...
  void _register() {
    setState(() {
      ...
      _departementError = null; _specialiteError = null;
    });
    ...
    if (selectedRole == 'Stagiaire') {
      ...
      // Encadrant : département et spécialité obligatoires (exigés par le backend)
      if (_departementController.text.trim().isEmpty) { _departementError = 'Le département est requis'; hasError = true; }
      if (_specialiteController.text.trim().isEmpty)  { _specialiteError  = 'La spécialité est requise'; hasError = true; }
    }

    if (hasError) { setState(() {}); return; }

    if (selectedRole == 'Encadrant') {
      // ✅ Encadrant → demande d'accès (validation admin requise)
      context.read<AuthBloc>().add(DemandeEncadrantSubmitted(
  ...
  Widget build(BuildContext context) {
    return BlocListener<AuthBloc, AuthState>(
      listener: (context, state) {
        ...
        } else if (state is DemandeSubmitted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          ...
          child: SingleChildScrollView(
            ...
            child: Column(
              ...
              children: [
                  ...
                  Container(
                    ...
                    child: const Row(
                      children: [
                        ...
                        Expanded(
                          child: Text(
                            'Votre demande sera validée par un administrateur avant activation.',
                            style: TextStyle(
                              fontSize: 12,
                              color: AppTheme.textDark,
                              fontFamily: 'Poppins',
                ...
                // ── Date de naissance (stagiaire uniquement) ──
                if (selectedRole == 'Stagiaire') ...[
                  const SizedBox(height: 16),
                  LoginField(
                ...
                if (selectedRole == 'Encadrant') ...[
                  const SizedBox(height: 24),
                  _SectionTitle(text: 'Informations professionnelles'),
                  const SizedBox(height: 12),
                  LoginField(
                    controller: _departementController,
                    label: 'Département',
                    hint: 'Ex : Développement, Réseaux...',
                    errorText: _departementError,
                  ),
                  const SizedBox(height: 16),
                  LoginField(
                    controller: _specialiteController,
                    label: 'Spécialité',
                    hint: 'Ex : Génie logiciel, IA...',
                    errorText: _specialiteError,
                  ),
                ],
